...
class _SignState extends State<travelSignUp> {
  ...
  var travelNameController = TextEditingController();
  ...
  selectTravelName() async {
    var res = await http.post(Uri.parse(TravelApi.travelList));

    if (res.statusCode == 200) {
      var data = json.decode(res.body);

      if (data['success'] == true) {
        List<dynamic> hotelInfoData = data['travel_info_data'];

        for (var hotel in hotelInfoData) {
          if (hotel['agency_name'] != null && hotel['agency_name'].isNotEmpty) {
            setState(() {
              travelNames.add(hotel['agency_name']);
            });
          }
        }
    ...
    try {
      var res = await http.post(
        ...
        body: {
          ...
          "agency_name": selectedValue.toString().trim(),
          "travel_tel": telController.text.trim(),
          "travel_name": travelNameController.text.trim(),
        },
      );
      print(res.body);
      if (res.statusCode == 200) {
